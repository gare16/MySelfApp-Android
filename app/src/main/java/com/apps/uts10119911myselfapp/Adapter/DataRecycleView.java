package com.apps.uts10119911myselfapp.Adapter;

// Created by Mochamad Tegar
// 5/25/2022 4:38 PM

import com.apps.uts10119911myselfapp.R;

public class DataRecycleView {

    public static String[] nameActivity = new String[]{
            "Jogging",
            "Eat",
            "Read Comic",
            "Play Games",
            "Coding",
            "Tasks"
    };

    public static int[] iconActivity = new int[]{
            R.drawable.ic_jogging,
            R.drawable.ic_food,
            R.drawable.ic_comic,
            R.drawable.ic_joystick,
            R.drawable.ic_binary_code,
            R.drawable.ic_daily_tasks
    };


    public static String[] nameFriends = new String[]{
            "Abdul",
            "Aziz",
            "Komar",
            "Rudin",
            "Alex",
            "Lucy",
            "Dora",
            "Andre",
    };

    public static int[] imageFriends = new int[]{
            R.drawable.fl1,
            R.drawable.fl2,
            R.drawable.fl3,
            R.drawable.fl4,
            R.drawable.fl5,
            R.drawable.fl6,
            R.drawable.fl7,
            R.drawable.fl8
    };

    public static int[] galerryCartoon = new int[]{
            R.mipmap.sponge1,
            R.mipmap.sponge2,
            R.mipmap.sponge3,
            R.mipmap.sponge4,
            R.mipmap.sponge5,
            R.mipmap.sponge6,
            R.mipmap.pat1,
            R.mipmap.pat2,
            R.mipmap.pat3,
            R.mipmap.squid1,
            R.mipmap.squid2,
            R.mipmap.squid3

    };
    public static String[] playlistSong = new String[]{
            "24K Magic - Bruno Mars",
            "Calling All My Lovelies - Bruno Mars",
            "Chunky - Bruno Mars",
            "Count On Me - Bruno Mars",
            "Finesse - Bruno Mars",
            "Gorilla - Bruno Mars",
            "Grenade - Bruno Mars",
            "It Will Rain - Bruno Mars",
            "Just The Way You Are - Bruno Mars",
            "Liquor Store Blues - Bruno Mars",
            "Locked Out of Heaven - Bruno Mars",
            "Marry You - Bruno Mars",
            "21 Guns - Green Day",
            "21st Century Breakdown - Green Day",
            "80 - Green Day",
            "86 - Green Day",
            "99 Revolutions - Green Day",
            "All By Myself - Green Day",
            "American Eulogy - Green Day",
            "American Idiot - Green Day",
            "Amy - Green Day",
            "Are We The Waiting - Green Day",
            "Armatage Shanks - Green Day",
            "At The Library - Green Day",
            "Bang Bang - Green Day"
    };
    public static String[] releasedSong = new String[]{
            "2009",
            "1992",
            "1995",
            "2012",
            "1994",
            "2009",
            "2004",
            "2012",
            "2004",
            "1995",
            "1990",
            "2016",
            "2004",
            "2012",
            "2004",
            "1995",
            "1990",
            "2016",
            "2004",
            "2012",
            "2004",
            "1995",
            "1990",
            "2016",
            "2010"
    };

}
